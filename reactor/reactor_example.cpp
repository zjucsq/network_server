  ...
  while (1) {
    ...
    // 5.有事件准备好
      ...
      for (i = 0; i < nready; ++i) {
        ...
        // 如果事件可读
        if ((ep_events[i].events & EPOLLIN) && (ev->events & EPOLLIN)) ev->callback(ev->fd, ev->events, ev->arg);

        // 如果事件可写
        if ((ep_events[i].events & EPOLLOUT) && (ev->events & EPOLLOUT)) ev->callback(ev->fd, ev->events, ev->arg);
      }
    }
  }
...
int accept_callback(int fd, int events, void *arg) {
  ...
  do {
    ...
    for (i = 5; i < MAX_EPOLL_EVENTS; ++i) {
      if (reactor->events[i].status == 0) break;
    }
  ...
  printf("New connect: [%s:%d], [time:%ld], pos[%d]\n", inet_ntoa(cli_addr.sin_addr), ntohs(cli_addr.sin_port),
         reactor->events[cli_fd].last_active, i);
